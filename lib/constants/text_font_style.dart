import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:google_fonts/google_fonts.dart';

import 'app_colors.dart';

class TextFontStyle {
  TextFontStyle._();

  static var headline24w700c0CC0DF = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 24.sp,
    fontWeight: FontWeight.w700,
  );
  static var headline24w500c0CC0DF = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 24.sp,
    fontWeight: FontWeight.w500,
  );
  static var textStyle26w700ccFFFFFF = GoogleFonts.poppins(
    color: AppColors.cFFFFFF,
    fontSize: 26.sp,
    fontWeight: FontWeight.w700,
  );
  static var textStyle16w300cF5F5F5 = GoogleFonts.poppins(
    color: AppColors.cF5F5F5,
    fontSize: 16.sp,
    fontWeight: FontWeight.w300,
  );
  static var textStyle26w700cF5F5F5 = GoogleFonts.poppins(
    color: AppColors.cF5F5F5,
    fontSize: 26.sp,
    fontWeight: FontWeight.w700,
  );
  static var textStyle24w700cF5F5F5 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 24.sp,
    fontWeight: FontWeight.w700,
  );
  static var textStyle16w400c767676 = GoogleFonts.poppins(
    color: AppColors.c767676,
    fontSize: 16.sp,
    fontWeight: FontWeight.w400,
  );
  static var textStyle16w500cFFFFFF = GoogleFonts.poppins(
    color: AppColors.cFFFFFF,
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
  );

  static var textStyle24w700c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 24.sp,
    fontWeight: FontWeight.w700,
  );
  static var textStyle16w400c5C5C5C = GoogleFonts.poppins(
    color: AppColors.c5C5C5C,
    fontSize: 16.sp,
    fontWeight: FontWeight.w400,
  );
  static var textStyle16w400c00BFA6 = GoogleFonts.poppins(
    color: AppColors.primaryColor,
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );
  static var textStyle16w400c33333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );
  static var textStyle16w500c00BFA6 = GoogleFonts.poppins(
    color: AppColors.primaryColor,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );
  static var textStyle16w500c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
  );
  static var textStyle20w500c333333 = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 20.sp,
    color: AppColors.c333333,
    fontWeight: FontWeight.w500,
    fontStyle: FontStyle.normal,
  );
  static var textStyle14w400c333333helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 14.sp,
    color: AppColors.c333333,
    fontWeight: FontWeight.w400,
    fontStyle: FontStyle.normal,
  );
  static var textStyle14w400c767676helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 14.sp,
    color: AppColors.c767676,
    fontWeight: FontWeight.w400,
    fontStyle: FontStyle.normal,
  );
  static var textStyle16400c333333helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.c333333,
    fontWeight: FontWeight.w400,
    fontStyle: FontStyle.normal,
  );
  static var textStyle16w700primaryColor = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.primaryColor,
    fontWeight: FontWeight.w700,
    fontStyle: FontStyle.normal,
  );
  static var textStyle14w700c00BFA6helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.cButtonColor,
    fontWeight: FontWeight.w700,
    fontStyle: FontStyle.normal,
  );
  static var textStyle16w500cFF6B6Bhelvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.cFF6B6B,
    fontWeight: FontWeight.w500,
    fontStyle: FontStyle.normal,
  );
  static var textStyle16w500primaryColor = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.primaryColor,
    fontWeight: FontWeight.w500,
    fontStyle: FontStyle.normal,
  );
  static var textStyle16w400c333333 = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.c333333,
    fontWeight: FontWeight.w400,
    fontStyle: FontStyle.normal,
  );
  static var textStyle14w500c00BFA6 = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 14.sp,
    color: AppColors.c00BFA6,
    fontWeight: FontWeight.w500,
    fontStyle: FontStyle.normal,
  );
  static var textStyle20w500c222222 = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    color: AppColors.c222222,
    fontSize: 20.sp,
    fontWeight: FontWeight.w500,
  );

//old------------->>>>

  static var textStyle12w500c0CC0DF = GoogleFonts.poppins(
    color: AppColors.c0CC0DF,
    fontSize: 12.sp,
    fontWeight: FontWeight.w500,
  );

  static var textStyle16w700c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 16.sp,
    fontWeight: FontWeight.w700,
  );

  static var textStyle12w400cA1A1A1 = GoogleFonts.poppins(
    color: AppColors.cA1A1A1,
    fontSize: 12.sp,
    fontWeight: FontWeight.w400,
  );

  static var textStyle13w400c818181 = GoogleFonts.poppins(
    color: AppColors.c818181,
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );

  static var textStyle14w400c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 14.sp,
    fontWeight: FontWeight.w400,
  );
  static var textStyle14w500c242424 = GoogleFonts.poppins(
    color: AppColors.c242424,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );

  static var textStyle16w600cFFFFFF = GoogleFonts.poppins(
    color: AppColors.cFFFFFF,
    fontSize: 16.sp,
    fontWeight: FontWeight.w600,
  );

  static var textStyle16w700cFFFFFF = GoogleFonts.poppins(
    color: AppColors.c000000,
    fontSize: 16.sp,
    fontWeight: FontWeight.w700,
  );

  static var textStyle18w500c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );
  static var textStyle20w600cFFFFFF = GoogleFonts.poppins(
    color: AppColors.cFFFFFF,
    fontSize: 18.sp,
    fontWeight: FontWeight.w500,
  );

  static var textStyle16w500cFF6B6B = GoogleFonts.poppins(
    color: AppColors.cFF6B6B,
    fontSize: 16.sp,
    fontWeight: FontWeight.w500,
  );


  static var textStyle16w500cFEFEFE = GoogleFonts.poppins(
    color: AppColors.cFEFEFE,
    fontSize: 14.sp,
    fontWeight: FontWeight.w500,
  );

  static var textStyle18w700c333333 = GoogleFonts.poppins(
    color: AppColors.c333333,
    fontSize: 18.sp,
    fontWeight: FontWeight.w700,
  );

  static var textStyle16w400c767676helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.c767676,
    fontWeight: FontWeight.w400,
    fontStyle: FontStyle.normal,
  );

  static var textStyle16w500c767676helvatica = TextStyle(
    fontFamily: 'HelveticaNeueRoman.otf',
    fontSize: 16.sp,
    color: AppColors.c767676,
    fontWeight: FontWeight.w500,
    fontStyle: FontStyle.normal,
  );


}
